...
PECS：producer-extends, consumer-super。如果参数化类型表示一个生产者T，就使用 `<? extends T>`；如果表示一个消费者T，就是用 `<? super T>`。

producer指的是向当前方法提供数据的对象，比如如下代码：
...
该方法的参数就是一个producer。consumer与producer相反，指的是接收数据的对象。
