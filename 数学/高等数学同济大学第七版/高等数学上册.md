...
弧微分公式： $\mathrm{d}s = \sqrt{1 + y^{{\prime}2}} \mathrm{d}x$。

### 二、曲率及其计算公式

用比值 $\frac{|\Delta \alpha|}{|\Delta s|}$，即单位弧段上切线转过的角度的大小来表达弧段的平均弯曲程度，这个比值叫做弧段的平均曲率，记作 $\overline{K}$，即 $\overline{K} = |\frac{\Delta \alpha}{\Delta s}|$。

当 $\Delta s \to 0$时，平均曲率的极限叫做曲线在点处的曲率，记作 $K$，即 $K = \lim \limits_{\Delta s \to 0} |\frac{\Delta \alpha}{\Delta s}|$。 $K$也可表示为 $K = |\frac{\mathrm{d} \alpha}{\mathrm{d}s}|$。

- 直线上任意点处的曲率都等于0。
- 圆上各点处的曲率都等于半径a的倒数 $\frac{1}{a}$。圆的弯曲程度到处都一样，半径越小曲率越大，即圆弯的越厉害。

便于实际计算的曲率公式：

- $y^{\prime} = \tan \alpha$ 
- $y^{\prime \prime} = \sec^2 \alpha \frac{\mathrm{d} \alpha}{\mathrm{d}x}$，故 $\frac{\mathrm{d} \alpha}{\mathrm{d}x} = \frac{y^{\prime \prime}}{1 + \tan^2 \alpha} = \frac{y^{\prime \prime}}{1 + y^{\prime 2}}$，于是 $\mathrm{d} \alpha = \frac{y^{\prime \prime}}{1 + y^{\prime 2}} \mathrm{d}x$
- 又 $\mathrm{d}s = \sqrt{1 + y^{\prime 2}} \mathrm{d}x$
- 则 $K = \frac{|y^{\prime \prime}|}{(1 + y^{\prime 2})^{\frac{3}{2}}}$。
